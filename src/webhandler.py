...
def find_needed_element(source_code: str):
...
def parse_list(current_date_column: ResultSet):
...
def get_ppr_list(driver: webdriver.Chrome):
    ...
    driver.close()
    current_date_column = find_needed_element(source_code)
    if len(current_date_column) == 0:
        return None
    else:
        return parse_list(current_date_column)
